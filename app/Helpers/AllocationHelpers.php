...
class AllocationHelpers{
    ...
    public static function getAllocatedSeedDetail($user, $package = null)
    {
        ...
        // ✅ Convert all category amounts in one step
        $table = self::convertSeedTable($user, $table);

        $total = array_sum($table);

        $seed = [
            'savings' => ($table['savings']) ? round(($table['savings'] / $total) * 100) : 0,
            'education' => ($table['education']) ? round(($table['education'] / $total) * 100) : 0,
            'expenditure' => ($table['expenditure']) ? round(($table['expenditure'] / $total) * 100) : 0,
            'discretionary' => ($table['discretionary']) ? round(($table['discretionary'] / $total) * 100) : 0
        ];

        $seed_web = array_values($seed);
    ...
    */
    public static function averageSeedDetail($user)
    {
        ...
        $target_currency = $preferred_currency ?? $calculator->currency ?? 'USD';
        ...
        foreach ($periodic_seeds as $period => $p_seeds) {
            $historic_seed[$period] = self::getSeedAverage($user, $calculator, $p_seeds, $total_seeds, $target_currency);
        }

        $average_seed = self::getSeedAverage($user, $calculator, $seeds, $total_seeds, $target_currency);
    ...
    public static function averageSeedPhilantrophy($user){
        ...
        $values = array_map(fn($val) => GapExchangeHelper::convert_currency($user, $philantrophy->currency ?? 'USD', $val), $values);
        return compact('labels', 'values');
    }
    ...
    private static function getSeedAverage($user, $calculator, $seeds, $total_seeds = 1, $target_currency = '')
    {
        $savings = [];
        $education = [];
        $expenditure = [];
        ...
        // ✅ Currency conversion (including total) handled here
        $savings = GapExchangeHelper::convert_currency($user, $target_currency, $savings);
        $education = GapExchangeHelper::convert_currency($user, $target_currency, $education);
        $expenditure = GapExchangeHelper::convert_currency($user, $target_currency, $expenditure);
        $discretionary = GapExchangeHelper::convert_currency($user, $target_currency, $discretionary);

        $table = compact('savings', 'education', 'expenditure', 'discretionary');
        $total = array_sum($table); // Sum the already-converted values
    ...
    private static function convertSeedTable($user, $table)
    {
        // Get preferred or calculator currency
        $preference = UserSetting::where('user_id', $user->id)
                            ->where('setting_key', 'preferences')
                            ->first();
        ...
        // Convert each amount in table
        foreach ($table as $key => $value) {
            $table[$key] = GapExchangeHelper::convert_currency($user, $target_currency, $value);
        }
    ...
    public static function monthlySeedDetail($user, $period)
    {
        ...
        $seed = self::convertSeedTable($user, $seed);
