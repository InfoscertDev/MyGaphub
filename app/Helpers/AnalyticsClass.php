...
class AnalyticsClass {
    ...
    {
        $audit = UserAudit::where('user_id', $user->id)
                    ->select('is_allocated')
                    ->first();
        ...
        if ($audit && $audit->is_allocated) {

            $liability = Liability::where('user_id', $user->id)
                            ->where('isArchive', 0)
                            ->where('credit_id', 1)
                            ->latest()
                            ->get();
            ...
            if ($credit) {
                $credit->baseline = $allocated['user_baseline'] ?? 0;
                $credit->current = $allocated['user_current'] ?? 0;
            }
        }

        if ($debt && !$debt->isArchive) {

            $primary_resident = Mortgage::where('user_id', $user->id)
                                    ->where('secured_against','Primary Residential Home')
                                    ->where('isArchive', 0)
                                    ->first();

            if ($primary_resident) {
                $debt->baseline = $primary_resident->open_balance ?? 0;
                $debt->current = $primary_resident->current_balance ?? 0;
            }
        }

        if ($freedom) {
            $freedom->target = $fin['cost'] ?? 0;
            $freedom->current = $fin['portfolio'] ?? 0;
        }

        if ($grand && empty($grand->main)) {
            $grand->current = $fin['calculator']->charity ?? 0;
        }
